...
import powerPoint from "/pptx-file.png";
...
const PowerPointToPdf = () => {
  ...
  const createPdf = () => {
    ...
    const url = `${import.meta.env.VITE_BACKEND_URL}/pdf/convert_to_pdf/pptx`;
    console.log(url);
    ...
    axios
      ...
      .catch((error) => {
        if (error.response) {
          ...
          console.error("Error Creating PDF:", error.response.data);
        } else if (error.request) {
          // No response received from the server
          console.error(
            "Error Creating PDF: No response received",
            error.request
          );
        } else {
          // Error setting up the request
          console.error(
            "Error Creating PDF: Request setup failed",
            error.message
          );
        }
      })
        ...
        <div className="flex items-center justify-center rounded-lg w-[100%] p-0 min-h-screen">
          {items.length === 0 ? (
              ...
              <div className="flex flex-col min-h-screen">
                <div className="flex flex-col items-center justify-center mt-8 px-4 sm:px-8 md:px-16 lg:px-24">
                  ...
                  <h2 className="text-lg sm:text-xl md:text-2xl text-gray-700 text-center mt-4">
                    Convert PowerPoint presentations to PDF in seconds.
                  </h2>
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
                    ...
                    <label
                      htmlFor="fileInput"
                      className="p-8 sm:p-6 md:p-8 bg-purple-600 sm:w-[400px] text-center max-w-xs hover:bg-purple-700 rounded-xl text-white font-bold text-lg sm:text-xl md:text-2xl cursor-pointer"
                    >
                      Select PowerPoint <br />
                      Presentations
                    </label>
            ...
            <div className="relative bg-[#f2f2f2] pt-0 w-screen min-h-screen">
              {/* <div className="bg-purple-500 -mt-10 rounded justify-center items-center hover:cursor-pointer p-2 px-3 absolute top-5 right-5 lg:top-5 lg:right-80 text-white">
                <input
                  type="file"
                  accept=".ppt, .pptx"
                  multiple
              ...
              <div className="flex flex-col lg:flex-row justify-between gap-10 lg:gap-40 mt-[2rem] relative">
                ...
                >
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className={`flex items-start relative justify-start h-[auto] w-32 rounded-md p-1 border-[#b9b9b9]  
                  ...
                  <div className="px-[0rem] py-[3rem] flex flex-col gap-[1rem] w-[100%] items-center justify-center">
                    ...
                    <div className="px-[1.5rem] ml-auto mr-auto rounded-md bg-[#d4edf6] py-[1.5rem] w-[90%]">
                      <p className="text-[1rem] font-normal">
                        To change the order of your PPTs, drag and drop the
                        files as you want.
                      </p>
                    </div>
                  </div>
