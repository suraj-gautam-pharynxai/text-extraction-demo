...
const ExtractPDF = () => {
  ...
  const [ranges, setRanges] = useState([{ id: 0, fromPage: "0", toPage: "" }]);
  ...
  const handleChange = (rangeId, field, value) => {
    ...
    if (field === "from") {
      ...
      setRanges((prevRange) =>
        prevRange.map((rangeChunk, i) =>
          i === rangeId
            ? { ...rangeChunk, fromPage: adjustedValue.toString() }
            : rangeChunk
        )
      );
    }
    ...
    if (field === "to") {
      ...
      setRanges((prevRange) =>
        prevRange.map((rangeChunk, i) =>
          i === rangeId
            ? { ...rangeChunk, toPage: adjustedValue.toString() }
            : rangeChunk
        )
      );
  ...
  const addRange = () => {
    setRanges((prevRanges) => [
      ...prevRanges,
      {
        id: prevRanges.length,
        fromPage: prevRanges[0]?.toPage,
        toPage: totalPagesNo,
      },
    ]);
    setSplitedChunk((prevApiPages) => [...prevApiPages, apiPages]);
  };
  ...
  useEffect(() => {
    if (apiPages?.length > 0) {
      if (typeof apiPages === "object" && apiPages !== null) {
        setSplitedChunk((prevChunks) => [...prevChunks, apiPages]);
      } else {
  ...
  const createPdf = async () => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        link.href = url;
        link.download = isChecked ? "converted.pdf" : "converted.zip";
  ...
  // Handle input change to update selected pages
                        ...
                        newRanges.map((chunkedPages, index) => (
                          ...
                          >
                            ...
                            <div className="text-center rounded-lg bg-gray-100 shadow-md rounded-sm relative p-1.5">
                              <img
                                src={
                                  splitedChunk[index]?.[chunkedPages.toPage - 1]
                                }
    ...
    <div className="w-full px-4 md:px-8">
      ...
      {modeExtractPages === "extract_all_pages" && (
        <>
          <div className="bg-blue-100 p-6 mt-5 rounded-sm">
            <p className="font-semibold">
              Selected pages will be converted into separate PDF files. {pages.length} PDF(s) will be created.
            </p>
          </div>
        </>
      )}
